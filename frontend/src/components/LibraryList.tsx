...
import { averageScore } from '../api/types';
...
function EntryRow({ entry, onRemove }: { entry: LibraryEntry; onRemove: () => void }) {
  const avg = averageScore(entry.ratings);
  return (
    <li className="flex gap-4 rounded-lg bg-slate-900 p-4 ring-1 ring-slate-800">
      ...
      <div className="flex flex-1 flex-col justify-between gap-3">
        <div className="flex items-start justify-between gap-3">
          ...
          {avg !== null ? (
            <span
              className="inline-flex shrink-0 items-center gap-1 rounded-md bg-slate-800 px-2 py-1 text-sm text-amber-300 ring-1 ring-slate-700"
              aria-label={`Average score ${avg.toFixed(1)} of 5`}
            >
